...
router.post('/register_admin', async (req, res) => {
    ...
    if (errors.length > 0) {
        ...
        if (query_result.length > 0) {
            ...
            const insert_result = await db_person_api.insertUserWithRole(name, phoneNumber, hashedPass, 'admin')
...
router.post('/register_company', async (req, res) => {
    ...
    if (errors.length > 0) {
        ...
        if (query_result.length > 0) {
            ...
            const person_entries = await db_person_api.insertUserWithRole(name, phoneNumber, hashedPass, 'owner')
...
router.get('/register_driver', (req, res) => {
    res.render('layout.ejs', {
        title: 'Register',
        body: 'auth/register',
        partials: '../partials/messages',
        formPostUrl: '/auth/register_driver'
    });
})

router.post('/register_driver', async (req, res) => {
    const {name, phone_number: phoneNumber, password, password2, licence} = req.body;
    let errors = [];

    if (!name || !phoneNumber || !password || !password2 || !licence) {
        errors.push({
            message: 'Please fill in the form'
        })
    }
    ...
    if (errors.length > 0) {
        ...
        if (query_result.length > 0) {
            ...
            const person_entries = await db_person_api.insertUserWithRole(name, phoneNumber, hashedPass, 'driver')
            console.log(person_entries);

            if (person_entries.p_id.length > 0) {
                // insert into driver
                const person_id = person_entries.p_id[0];
                const driver_entries = await db_driver_api.insertDriver(person_id, licence)
                if (driver_entries.rowsAffected > 0){
                    req.flash('success_msg', 'Registered user! Login to view the page')
                    res.redirect('/auth/login')
                }else {
                    errors.push({
